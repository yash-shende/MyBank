...
tr:nth-child(even):hover {background-color: #bdc3c7;}
    ...
    <title>Admin Request Withdraw Money</title>
...
<li><a href="adminATMapprove.php">ATM Card</a></li>
</ul>


<div style="margin-left:17%;margin-top:5%;padding:1px 16px;height:100px;">

<p>This page shows the requests from the users who want to withdraw money from their account. Only when the Admin grants
   them the permission,<br> the user can withdraw it.</p>
<h3>Requests from the user for withdrawing money from their account:</h3>
<table>
<tr>
<td>CodeNo</td>
...
<?php 
$mess="";
$sql="SELECT * FROM duptransaction WHERE type1='withdraw'";
...
if(mysqli_query($conn,$sql))
{
    if(mysqli_num_rows($result) > 0)
    {
        ...
        {
            echo "<tr><td>".$row["id"]."</td><td>".$row["remacno"]."</td><td>".$row["name1"]."</td><td>".$row["debit"]."</td><td>".$row["date1"];
        }
        echo "</table>";
    }    
    else{
...
<?php
  if(isset($_POST["approve"]))
  {
    ...
    $sql="SELECT id FROM duptransaction WHERE type1='withdraw' ";
    $result=mysqli_query($conn,$sql);
    if($result)
    {
        while($row=mysqli_fetch_assoc($result))
...
if($flag==1)
{      
//here code selects the acno and amount to be withdrawn into realtransaction
...
$upd1=$upd-$with; 
      ...
      $sql="UPDATE realtransaction SET debit='$with' , updated='$upd1' WHERE remacno='$remacno'";
      if (mysqli_query($conn, $sql))
    {
...
$insert="INSERT INTO transactionrecord (remacno,credit,debit,updated,details,date1)
         VALUES ('$remacno','$add','$with','$upd','DEBIT','$today')";
...
if(mysqli_query($conn,$del))
{
 echo '<script type="text/javascript">';
    echo 'window.location.href="adminWithMoney.php";';
    echo '</script>';
}
...
<div class="col-xs-3">
<form method="POST" >
<input type="number" name="id" class="form-control" min="0" oninput="validity.valid||(value='');" placeholder="Enter here" reqiured><br>
<input type="submit" name="approve" value="Approve" class="btn btn-warning">
</form>
</div>
<br><br>
