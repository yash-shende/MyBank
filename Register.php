   ...
   <link rel="stylesheet" href="styleRegister.css">
    ...
    <script>
      function checkAccNum(){
        ...
        let acc_num = document.getElementsByName("acNo")[0].value;
        if(acc_num.length != 12){
          document.getElementById("submit-btn").disabled = true; 
          document.getElementsByName("acNo")[0].style="border:solid 4px red;"        
          window.alert("Account Number should be of 12 digits  ") ;
        }   
        else{
          document.getElementById("submit-btn").disabled = false; 
          document.getElementsByName("acNo")[0].style="border:solid 0px ;" 
        }     
      }
      function dateValidator(){
          ...
          let inpYear = Number(dob.substr(0,4));
          
          let Age = cYear-inpYear;  

          if(Age < 18){
          document.getElementById("submit-btn").disabled = true; 
          document.getElementById("dob").style="border:solid 4px red;"       
          window.alert("You are Not eligible ");   
          }  
          else{
            document.getElementById("submit-btn").disabled = false; 
            document.getElementById("dob").style="border:solid 0px ;" 
          }
        }
    </script>
...
<center><h1 id="heading">Sign Up</h1></center>
  ...
  <form class="form-group" name="RegisterForm" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
     ...
     <label>E-mail:</label><span class="error">*</span>
      ...
      <label>Bank A/C No:</label><span class="error">*</span>
        <input type="number"  onchange="checkAccNum()" name="acNo" placeholder="Bank A/C No here" class="form-control" maxlength="12" required>
  ...
  <div class="row">          
      ...
      <div class="row">               
          <div class="col-lg-6">
             ...
             <input type="date"  name="DOB" id="dob" onchange="dateValidator()" class="form-control" required>
          </div> 
    ...
    <br>
    <input type="submit" id="submit-btn" name="submit" value="Submit" class="btn btn-primary btn-md btn-block" data-toggle="modal" data-target="alreadyRegistered">
            </form>
          </div>
       </div>
       <div class="col-lg-3"></div>
